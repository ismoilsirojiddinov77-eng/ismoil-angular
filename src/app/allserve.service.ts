...
export class AllserveService {
  ...
  addToFav(item: ClothingItem) {
    // fav signalining hozirgi qiymatini olish
    const currentFav = this.fav();
    // item allaqachon borligini tekshirish
    const index = currentFav.findIndex(el => el.id === item.id);

    if (index > -1) {
      // mavjud itemni nusxa qilib olib, num qiymatini oshiramiz
      const updatedItem = { ...currentFav[index] };
      updatedItem.num = (updatedItem.num ?? 0) + 1;

      // yangi massiv yaratib signalni yangilaymiz
      this.fav.set([
      ...
      // yangi itemni num=1 bilan qo‘shamiz
